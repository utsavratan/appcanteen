...
export const createAnnouncement = async (ann: Partial<Announcement>) => {
  const { data, error } = await db.from('announcements').insert(ann).select().single();
  if (error) throw error;
  return data;
};
...
export const fetchAuditLogs = async (limit = 100): Promise<AuditLog[]> => {
