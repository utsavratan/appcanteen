...
import { createOrder, createOrderItems, addAuditLog } from '@/lib/db';
...
const Checkout = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      await addAuditLog('order_placed', 'customer', { order_id: order.id, payment_mode: paymentMode, grand_total: grandTotal, user_name: profile?.name });
    ...
    return (
      <div className="min-h-screen bg-background gradient-mesh flex items-center justify-center px-4">
        ...
        >
          ...
          {profile?.name && <p className="text-sm text-muted-foreground mt-1">Thank you, {profile.name}!</p>}
  ...
  return (
    <div className="min-h-screen bg-background gradient-mesh pb-8">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 space-y-4">
        {/* Customer Info */}
        {profile?.name && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass shadow-glass rounded-xl p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl gradient-primary flex items-center justify-center">
              <span className="text-primary-foreground font-bold text-sm">{profile.name.charAt(0).toUpperCase()}</span>
            </div>
            <div>
